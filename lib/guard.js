...
function isPrivateIPv6(address) {
  ...
  // The same address in hex form. `new URL()` normalises ::ffff:127.0.0.1 to
  // ::ffff:7f00:1, so without this the dotted-quad check above never sees it.
...
/**
 * Opt-in escape hatch for reaching private addresses, off unless explicitly
 * enabled. Useful for pointing the proxy at something on your own LAN; enabling
 * it on a public deployment hands anyone who can reach the proxy your internal
 * network, so server.js warns loudly at boot when it is on.
 */
function privateHostsAllowed() {
  return /^(1|true|yes)$/i.test(process.env.ALLOW_PRIVATE_HOSTS ?? '');
}
...
export async function assertSafeUrl(url) {
  ...
  if (privateHostsAllowed()) return parsed;
